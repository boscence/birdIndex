...
#########  
  bsplit = split(birds, birds$lga)

##########
#Create an empty dataframe
#We do this to create a home for the output of the bird index function
#########  
  bi = data.frame(YearGroup = factor(), LGA = factor(), indexscore = integer()) 
...
#########  
for (i in seq(bsplit)){
  mwLGA = data.frame(unique(names(bsplit)))
  #Count how many surveys were done at each lga and by each year group(surveysperLGA)
  surveysperLGA =  ddply(bsplit[[i]],.(lga,startYearGp),here(summarize),
                         surveysAtLGA = length(unique(surveyid)))
  LGAname = names(bsplit[i])
  ...
  birdindex = merge(surveysperLGA, birdindex, by.x = "startYearGp", by.y = "startYearGp")
  
  #Calculate the observed over expected
  birdindex$OE = birdindex$surveys_seen_at/birdindex$surveysAtLGA
  
  #Sums the observed over expected by lga and year group
  bindex = ddply(birdindex,.(lga,startYearGp),here(summarize),
                 LGA = unique(LGAname),
                 indexscore = sum(OE),
                 seenat = sum(surveys_seen_at),
                 surveysatLGA = sum(surveysAtLGA))
  
  bindex = bindex[,c(2,3,4)]
  bindex = bindex[complete.cases(bindex),]
  names(bindex) = c("YearGroup", "LGA", "indexscore")
  bindex = bindex[ which(bindex$YearGroup %in% c("1995 to 1999",
                                                 "2000 to 2004",
                                                 "2005 to 2009",
                                                 "2010 to 2014")), ]
  birdIndexPlot = qplot(YearGroup,indexscore,data = bindex,color = YearGroup,geom_point(colour=YearGroup),facets = ~LGA,
                        xlab = "",
...
birdIndexPlotAll = qplot(YearGroup,indexscore,data = MWBirdIndex,color = YearGroup,geom_point(colour=YearGroup),facets = ~LGA,
                      main = "Avian Sub-Index for LGAs within or near the Melbourne Water area",
                      xlab = "Year Group",
                      ylab = "Index Score") + 
...
###Write the CSV
write.csv(MWBirdIndex, file = "AviansubIndexScores.csv")

###Write the chart
ggsave(birdIndexPlotAll,filename="AvianIndexPlot All LGAs.png",width = 400, height = 400, units = "mm")
